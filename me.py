...
menu = ['About','Sales Image','Data Analysis','Machine Learning']
...
if selection== 'Machine Learning':
        ...
        # Turning categories into numbers
          ...
          def user_report():
               ...
               Town = st.sidebar.slider('Town',int(df2.Town.min()),int(df2.Town.max()),1)
               Address= st.sidebar.slider('Address', df2.Address.min(),df2.Address.max(),1)
               Assessed_Value= st.sidebar.number_input('Assessed_Value',df2.Assessed_Value.min())
               Property_Type=st.sidebar.slider('Property_Type', int(df2.Property_Type.min()),int(df2.Property_Type.max()),1)
               Residential_Type=st.sidebar.slider('Residential_Type', int(df2.Residential_Type.min()),int(df2.Residential_Type.max()),1) 
               Sale_Month=st.sidebar.slider('Sale_Month',int(df2.Sale_Month.min()),int(df2.Sale_Month.max()))
          ...
          Sale_Amount=model.predict(user_data)
          st.header('Property Sale Amount')
          st.header("$"+str(np.round(Sale_Amount[0],2)))